...
export default function Home() {
  ...
  // Efek Generator Kode Biner Acak (Matrix Flood) pas proses dekripsi
  useEffect(() => {
    ...
    // Akselerasi Loading Bar Super Cepat & Agresif
    const progressInterval = setInterval(() => {
      setProgress((prev) => {
        if (prev >= 100) {
          clearInterval(progressInterval)
          clearInterval(binaryInterval)
          setTimeout(() => setBootState('READY'), 400) // Masuk ke portfolio
          return 100
        }
        return prev + Math.floor(Math.random() * 8 + 4) // Melompat patah-patah
      })
    }, 60)
    ...
    <div>
      <AnimatePresence mode="wait">
        {bootState !== 'READY' ? (
          ...
          >
            {/* Scanline CRT Glass Distortion Effect */}
            ...
            <div className="w-full max-w-md border border-neutral-900 bg-neutral-950/80 p-6 relative space-y-6 backdrop-blur-md">
              ...
              {/* INTERACTIVE CONTROLLER CONSOLE */}
              <AnimatePresence mode="wait">
                {bootState === 'LOCKED' ? (
                  <motion.div 
                    key="lock-state"
          ...
          <motion.main 
            key="main-portfolio"
            initial={{ opacity: 0, filter: "brightness(0) blur(5px)", scale: 0.98 }}
